...
图例设置仅在默认启用此选项的时序小部件中进行配置才有意义。您可以在 [此处](/docs/{{docsPrefix}}user-guide/dashboards/#3-legend-settings) 阅读有关图例设置的更多信息。
