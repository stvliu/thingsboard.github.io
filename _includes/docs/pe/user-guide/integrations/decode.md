...
### 步骤 6. 设置 MQTT
在 MQTT 设置菜单中，勾选复选框：启用、清除会话和 JSON 格式。输入 Thingsboard 代理数据：代理：**demo.docs.codingas.com**，端口：**1883**，重新连接周期：**10**，保持活动间隔：**60**，客户端 ID：输入从 GridLinks 演示帐户获得的 **设备 ID**，用户名：输入从 GridLinks 演示帐户获得的 **访问令牌**，密码：留空。单击“保存”按钮。
