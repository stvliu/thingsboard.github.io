...
## 简介
本教程将演示如何使用 Decode DL28 通信处理器将来自区域供热分站 (DHS) 设施的各种信号设置并集成到 ThingsBoard IoT 平台中。来自数字和模拟 I/O、Modbus RTU 设备和 M-Bus 热量计的信号组合在一个 ThingsBoard 仪表板中。由于简单起见，只连接了少数信号和设备，但在实际安装中，最多可以使用数百个设备和数千个信号。
...
Decode [DL28](https://decode.rs/product/dl28/) 设备充当通信和信号集中器，并在安装在 DHS 中的所有控制、监控和测量设备与互联网上的 ThingsBoard 服务器之间提供桥接功能。此示例中使用了三个信号：两个继电器输出和一个来自热量计的温度。
...
## ThingsBoard 的设置
### 步骤 1. 登录
转到 ThingsBoard 演示网页 https://demo.thingsboard.io 并使用创建帐户时获得的凭据登录。
### 步骤 2. 添加 DL28 设备
...
单击“添加”按钮将小部件添加到仪表板。使用类似的过程，添加另一个开关小部件，其开关标题为 **GT900 RL1**、属性/时间序列值键为 **gt900_relay_1**，在“转换值函数”中，开关处于打开状态时为 **{ "gt900_relay_1": 1 }**，开关处于关闭状态时为 **{ "gt900_relay_1": 0 }:**。

使用类似的过程，添加两个 LED 指示器小部件以显示继电器状态，使用 LED 标题 **EXIO LED RL1** 和 **GT900 LED RL1**，以及属性/时间序列值键 **exio_relay_1** 和 **gt900_relay_1**。
...
使用类似的过程，添加数字仪表小部件以显示来自 M-Bus 热量计的回水温度，添加新的数据源。在“实体时间序列/属性”字段中输入 **cal_return_temp**，然后单击时间序列的符号。单击“添加”按钮将仪表添加到仪表板。
...
## 结论
事实证明，ThingsBoard 是远程监控和控制区域供热变电站设施的理想平台。该解决方案在增加设备、仪表板和小部件的数量方面具有很强的可扩展性，甚至可以覆盖整个城市的较大配电网络。还证明了在 ThingsBoard 系统中可以将远程监控和控制与抄表混合使用。
