...
// MQTT 客户端发送或接收的最大数据包大小，如果大小太小，则可能不会发送消息或接收的消息将被丢弃
...
/// @brief 如果已删除连接，则重新连接 WiFi 并使用 InitWiFi
...
/// 请参阅 https://arduinojson.org/v5/api/jsonvariant/subscript/ 了解更多详细信息
...
/// @brief 更新回调，将在提供的一个共享属性值更改后立即调用，
/// 如果没有提供，我们改为订阅任何共享属性更改
...
void loop() {
  ...
  if (!tb.connected()) {
    ...
    // 执行订阅。所有后续数据处理都将在
    // processSetLedState() 和 processSetLedMode() 函数中发生，
    // 如回调数组所示。
  ...
  // 每隔 telemetrySendInterval 时间发送遥测
