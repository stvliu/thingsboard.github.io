...
curl -v -X POST --data "{"attribute1": "value1", "attribute2":true, "attribute3": 43.0}" https://gridlinks.codingas.com/api/v1/ABC123/attributes --header "Content-Type:application/json"
...
# For example, $THINGSBOARD_HOST_NAME reference live demo server, $ACCESS_TOKEN is ABC123:
curl -v -X POST -d @new-attributes-values.json https://gridlinks.codingas.com/api/v1/ABC123/attributes --header "Content-Type:application/json"